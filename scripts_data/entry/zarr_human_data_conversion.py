...
def main(input_dir, output, instruction, calib_quest2camera_file, speed_downsample_ratio, hand_shrink_coef,
    ...
    if n_encoding_threads > 1:
        ...
        process_list = []
        for i in range(n_encoding_threads):
            p = Process(target=conversion_trajectory, args=(input_data_fp_batch_list[i], calib_quest2camera, speed_downsample_ratio, hand_shrink_coef, mode, out_resolution_resize, out_resolution_crop, resolution_image_final, num_points_final, points_max_distance_final, replay_buffer, network_delay_checking, i))
            p.start()
        ...
        conversion_trajectory(input_data_fp_list, calib_quest2camera, speed_downsample_ratio, hand_shrink_coef, mode, out_resolution_resize, out_resolution_crop, resolution_image_final, num_points_final, points_max_distance_final, replay_buffer, network_delay_checking, 0)
