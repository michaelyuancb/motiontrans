...
from common.sampler_dp import SequenceSampler, get_val_mask
...
class DPDataset(BaseDataset):
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        for i in range(self.num_robot):
            ...
            action_normalizers.append(get_identity_normalizer_from_stat(array_to_stats(data_cache['action'][..., i * dim_a + 3: (i + 1) * dim_a - 6]))) # rot
            action_normalizers.append(get_range_normalizer_from_stat(array_to_stats(data_cache['action'][..., (i + 1) * dim_a - 6: (i + 1) * dim_a])))  # gripper
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for robot_id in range(self.num_robot):
            ...
            pose_mat = pose_to_mat(np.concatenate([
                obs_dict[f'robot{robot_id}_eef_pos'],
                obs_dict[f'robot{robot_id}_eef_rot_axis_angle']
            ], axis=-1))   # robot-base proprioception
            action_mat = pose_to_mat(data['action'][...,12 * robot_id: 12 * robot_id + 6])  # robot-base action
            
            # solve relative obs
            obs_pose_mat = convert_pose_mat_rep(     # robot-base eef[T]-eef[T-1]
                pose_mat, 
                base_pose_mat=pose_mat[-1],
                pose_rep=self.obs_pose_repr,
                backward=False)
            action_pose_mat = convert_pose_mat_rep(  # robot-base action[T]-eef[T-1]
                action_mat, 
                base_pose_mat=pose_mat[-1],
                pose_rep=self.obs_pose_repr,
                backward=False)
            ...
            action_gripper = data['action'][..., 12 * robot_id + 6: 12 * robot_id + 12]
