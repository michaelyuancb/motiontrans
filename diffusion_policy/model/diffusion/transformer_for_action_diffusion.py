...
class TransformerForActionDiffusion(ModuleAttrMixin):
    def __init__(self,
        ...
        self.pos_emb = nn.Parameter(torch.randn((1, action_horizon + 1, n_emb)))
        ...
        self.cond_cross_attn_proj = nn.Linear(cond_dim, n_emb)

        # decoder head
        self.head = FinalDecoder(n_emb, output_dim)
    ...
    def forward(self, 
        ...
        cond_cross_attn_feat = self.cond_cross_attn_proj(cond)
        cond_emb = torch.cat([time_emb, cond], dim=1)  # (B, diffusion_step_embed_dim + cond_dim)
        
        # 3. process input
        input_emb = self.input_emb(sample)
        # Add condition cross attention feature to input embedding
        input_emb = torch.cat([input_emb, cond_cross_attn_feat[:, None]], dim=1)  # (B, T+1, n_emb)
        t = input_emb.shape[1]
