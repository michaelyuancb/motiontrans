...
class DynamixelDriver(DynamixelDriverProtocol):
    ...
    def read_joint(self):
        joint_position = np.zeros(len(self._ids), dtype=np.float64)
        for i, dxl_id in enumerate(self._ids):
            dxl_present_position, _, _ = self._packetHandler.read4ByteTxRx(self._portHandler, dxl_id,
                                                                                       ADDR_PRESENT_POSITION)
            joint_position[i] = dxl_present_position
        return joint_position / 2048.0 * np.pi
