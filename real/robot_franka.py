...
class RobotFrankaInterface:
    ...
    def start_joint_impedance(self, Kp: np.ndarray, Kd: np.ndarray):
    ...
    def ik_exist(self, pose: np.ndarray, q0: np.ndarray, tol: float = 0.001):
        position = pose[:3]
        orientation = st.Rotation.from_rotvec(pose[3:]).as_quat()
        final_ik_flag, result_desc, ik_solution = self.server.ik_exist(position.tolist(), orientation.tolist(), q0.tolist(), tol)
...
class RobotFranka(mp.Process):
    ...
    def start(self, wait=True):
        ...
        if wait:
            self.start_wait()
        if self.verbose:
            print(f"[FrankaPositionalController {self.robot_port}:{self.robot_port}] Controller process spawned at {self.pid}")
        self.t_last_schedule = np.inf
    ...
    def run(self):
        ...
        robot = RobotFrankaInterface(self.robot_ip, self.robot_port)

        init_eef_pose = None

        try:
            if self.verbose:
                print(f"[FrankaPositionalController {self.robot_port}:{self.robot_port}] Connect to robot: {self.robot_ip}")
            ...
            # # franka Kp 
            # self.Kp = np.array([300, 300, 300, 30, 30, 30])
            # self.Kd = np.array([30, 30, 30, 2.5, 2.5, 2.5])
            ...
            while keep_running:
                ...
                for i in range(n_cmd):
                    ...
                    if cmd == Command.STOP.value:
                        ...
                        print(f"SERVOL: {target_pose} {duration}")
                        curr_time = t_now + dt
                        ...
                        if self.verbose:
                            print(f"[FrankaPositionalController {self.robot_port}:{self.robot_port}] New pose target:{target_pose} duration:{duration}s")
                        self.t_last_schedule = time.monotonic()
                    elif cmd == Command.RESET.value:
                        if self.verbose:
                            print("RESET Franka")
                        target_pose = init_eef_pose
                        duration = self.joints_init_duration
                        curr_time = t_now + dt
                        t_insert = curr_time + duration
                        pose_interp = pose_interp.drive_to_waypoint(
                            pose=target_pose,
                ...
                if self.verbose:
                    print(f"[FrankaPositionalController {self.robot_port}:{self.robot_port}] Actual frequency {1/(time.monotonic() - t_now)}")
        except KeyboardInterrupt:
            robot.terminate_current_policy()
            del robot
            self.ready_event.set()
            if self.verbose:
                print(f"KeyboardInterrupt [FrankaPositionalController {self.robot_port}:{self.robot_port}] Disconnected from robot: {self.robot_ip}")
        finally:
            # manditory cleanup
            # terminate
            print(f'\n\n\n\nterminate_current_policy Franka {self.robot_ip}\n\n\n\n\n')
            robot.terminate_current_policy()
            del robot
            self.ready_event.set()

            if self.verbose:
                print(f"[FrankaPositionalController {self.robot_port}:{self.robot_port}] Disconnected from robot: {self.robot_ip}")
