...
class DynamixelRobot(Robot):
    ...
    # joint1 is limited into [-1/2*pi, 1/2*pi]
    # joint2 is limited into [-1/8*pi, 1/2*pi]
    def command_joint_state(self, joint_state: np.ndarray) -> None:
        if abs(joint_state[0]) - np.pi / 2 > 0 :
            raise ValueError(f"The joint position of active vision servo_id_1 is {joint_state[0]}. Out of range!")
        if joint_state[1] < -np.pi * 1 / 8 or joint_state[1] > np.pi * 1 / 2 :
            raise ValueError(f"The joint position of active vision servo_id_2 is {joint_state[1]}. Out of range!")
    ...
    def read_joint_state(self):
        joint_value = self._driver.read_joint()
        joint_value = (joint_value - self._joint_offsets).tolist()
        return joint_value
