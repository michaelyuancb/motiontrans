...
class SharedNDArray(Generic[SharedT]):
    ...
    def __init__(
        ...
        assert shm.size >= (dtype.itemsize * int(np.prod(shape)))
        self.shm = shm
        self.dtype = dtype
    ...
    def create_from_shape(
        ...
        # np.prod(shape) need to transfer to int first, otherwise may cause overflow error (e.g. change to negative number)
        if int(int(np.prod(shape)) * dtype.itemsize) < 0:
            raise ValueError(f"Negative size for shared memory: {int(int(np.prod(shape)) * dtype.itemsize)}, shape: {shape}, dtype: {dtype}, dtype.itemsize: {dtype.itemsize}")
        shm = mem_mgr.SharedMemory(int(int(np.prod(shape)) * dtype.itemsize))
