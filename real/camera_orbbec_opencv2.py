...
class CameraOrbbecOpencv2(mp.Process):
    """
    ...
    def __init__(
            ...
            recorder: Optional[RecorderRGBDVideo] = None,
            verbose=False,
            debug=False,
    ):
        super().__init__()
    ...
    # ========= orbbec sdk ============
    # @staticmethod
    # def get_video_stream(cap, wait_time=50):
    ...
    def run(self):
        ...
        #
        cap = cv2.VideoCapture(self.device_id, cv2.CAP_OBSENSOR)  # TODO: device id setting
        ...
        # st = time.time()
        # data_c, data_d = self.get_video_stream(cap)
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                buffer_image, buffer_depth = self.recorder.get_img_buffer()

                if cap.grab():
                    ret, buffer_image = cap.retrieve(buffer_image, cv2.CAP_OBSENSOR_BGR_IMAGE)
                    if (ret is None) or (not ret):
                        print(f"[Orbbec Camera {np.round(time.time())}] failed to get color frame")
                        continue
                    ret, buffer_depth = cap.retrieve(buffer_depth, cv2.CAP_OBSENSOR_DEPTH_MAP)
                    if (ret is None) or (not ret):
                        print(f"[Orbbec Camera {np.round(time.time())}] failed to get depth frame")
                        continue
                else:
                    print(f"[Orbbec Camera {np.round(time.time())}] failed to grab data")
                    continue
                ...
                if self.recorder.is_ready():
                    self.recorder.write_img_buffer(buffer_image, buffer_depth, frame_time=t_cal)

                data = dict()
                data['camera_receive_timestamp'] = t_recv
                data['camera_capture_timestamp'] = t_cap
                data['rgb'] = buffer_image
                data['depth'] = buffer_depth
