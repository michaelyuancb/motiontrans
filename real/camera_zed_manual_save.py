...
class CameraZed(mp.Process):
    ...
    def __init__(
            ...
            recorder_stereo_right: Optional[Callable[[Dict], Dict]] = None,
            enable_vis=False,
            verbose=False,
        ):
        super().__init__()
        ...
        if recorder is None:
            if enable_pointcloud is True:
                ...
                recorder = RecorderRGBVideo.create_h264(
                    ...
                    thread_type='FRAME',
                    thread_count=1)
        if recorder_stereo_right is None:
        ...
        self.recorder_stereo_right = recorder_stereo_right
    ...
    def start(self, wait=True, put_start_time=None):
        ...
        if self.enable_pointcloud:
            data_example['pointcloud'] = np.empty(shape=shape+(3,), dtype=np.float32)
        ...
        self.recorder_stereo_right.start(
            shm_manager=self.shm_manager, 
            data_example=data_example)
    ...
    def stop(self, wait=True):
        self.recorder.stop()
        self.recorder_stereo_right.stop()
        self.stop_event.set()
        if wait:
            self.end_wait()
    ...
    def start_wait(self):
        ...
        self.recorder_stereo_right.start_wait()
    
    def end_wait(self):
        self.join()
        self.recorder.end_wait()
        self.recorder_stereo_right.end_wait()
    ...
    def get_cam_resolution(self):
        if self.capture_fps == 60:
            z_res = sl.RESOLUTION.HD720, 1280, 720
        elif self.capture_fps == 30:
            z_res = sl.RESOLUTION.HD1080, 1920, 1080
        elif self.capture_fps == 15:
            z_res = sl.RESOLUTION.HD2K, 2208, 1242
        return z_res
    ...
    def get_camera_info(self):
        ...
        camera_info['intrinsics_right'] = intrinsic_right
        camera_info['distortion_right'] = dist_right
        camera_info['height'] = self.get_cam_resolution()[2]
        camera_info['width'] = self.get_cam_resolution()[1]
        return camera_info
    ...
    def run(self):
        ...
        z_res, z_w, z_h = self.get_cam_resolution()
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                if self.recorder.is_ready() and self.recorder_stereo_right.is_ready():
                    self.recorder.write_frame(rec_data, frame_time=t_cal)
                    rec_data['rgb'] = rec_data['rgb_right']
                    self.recorder_stereo_right.write_frame(rec_data, frame_time=t_cal)
                ...
                for i in range(n_cmd):
                    ...
                    if cmd == Command.START_RECORDING.value:
                        ...
                        self.recorder_stereo_right.start_recording(video_path, start_time=start_time)
                    elif cmd == Command.STOP_RECORDING.value:
                        self.recorder.stop_recording()
                        self.recorder_stereo_right.stop_recording()
